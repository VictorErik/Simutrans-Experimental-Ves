...
sint32 rail_vehicle_t::activate_choose_signal(const uint16 start_block, uint16 &next_signal_index, uint32 brake_steps, uint16 modified_sighting_distance_tiles, route_t* route)
{
	...
	for(uint32 idx = start_block + 1; choose_ok && idx < route->get_count(); idx++)
	{
		...
		if(way->has_signal())
		{
			signal_t *sig = gr->find<signal_t>(1);
 			ribi_t::ribi ribi = ribi_typ(route->position_bei(max(1u, idx) - 1u));	
	...
	else
	{
		...
		blocks = block_reserver(route, start_block, modified_sighting_distance_tiles, next_signal_index, 100000, true, false, true, none, false, false, brake_steps);
		if(!blocks) 
		{
...
bool rail_vehicle_t::can_enter_tile(const grund_t *gr, sint32 &restart_speed, uint8)
{
	...
	bool reversed_at_this_stop = false;
	if(cnv->get_state() == convoi_t::CAN_START)
	{
		...
		reversed_at_this_stop = fpl->eintrag[fahrplan_index].reverse == 1;
		...
		{
			if(reversed_at_this_stop || working_method == token_block || working_method == one_train_staff)
			{
				working_method = drive_by_sight;
			}
		}
	...
	if(destination_is_nonreversing_waypoint || starting_from_stand && working_method != one_train_staff && (reversed_at_this_stop || (working_method != absolute_block && working_method != track_circuit_block && working_method != cab_signalling)))
	{
	...
	if(next_stop > last_index && !exiting_one_train_staff) // last_index is a waypoint and we need to keep routing.
	{
		...
		if(!weg_frei)
		{
			...
			if(!weg_frei)
			{
				...
				if(fpl->eintrag[fpl->get_aktuell()].reverse == 0)
				{
					...
					// transitional)cases in which next_stop is still ahead of the calculated route. 
		...
		if (weg_frei || next_stop > last_index)
			return true;
	}
	...
	if(working_method == absolute_block || working_method == track_circuit_block || working_method == drive_by_sight || working_method == token_block || working_method == time_interval || working_method == time_interval_with_telegraph)
	{
		...
		{
			const koord3d tile_to_check_ahead = route.position_bei(min(route.get_count() - 1u, route_index + tiles_to_check));
			const koord3d previous_tile = route.position_bei(min(route.get_count() - 1u, route_index + tiles_to_check) -1u);
			...
			uint16 modified_route_index = min(route_index + tiles_to_check, route.get_count() - 1u);
			ribi_t::ribi ribi = ribi_typ(route.position_bei(max(1u,modified_route_index)-1u), route.position_bei(min(route.get_count()-1u, modified_route_index+1u)));
			signal_t* signal = way->get_signal(ribi); 
	...
	{
		ribi_t::ribi ribi = route_index > route.get_count() ? ribi_t::alle : ribi_typ(route.position_bei(max(1u, route_index) - 1u), route.position_bei(min(route.get_count() - 1u, route_index + 1u)));
	...
	{
		bool ok = block_reserver(&route, route_index, modified_sighting_distance_tiles, next_signal, 0, true, false, false, none, false, false, brake_steps);
...
sint32 rail_vehicle_t::block_reserver(route_t *route, uint16 start_index, uint16 modified_sighting_distance_tiles, uint16 &next_signal_index, int count, bool reserve, bool force_unreserve, bool is_choosing, onward_reservation_type onward_reservation, bool is_from_starter, bool is_from_directional, uint32 brake_steps)
{
	...
	for( ; success && count >= 0 && i < route->get_count(); i++)
	{
		...
		if(reserve)
		{
			...
			{			 
				...
				if(signal)
				{
					...
					if(!signal->get_besch()->is_pre_signal()) // Stop signal or multiple aspect signal
					{
						...
						if(!directional_only && next_signal_working_method == time_interval_with_telegraph && signal->get_no_junctions_to_next_signal() && signal->get_besch()->is_longblock_signal())
						{
							directional_only = true;
						}

						if((!onward_reservation || first_stop_signal_index == INVALID_INDEX) && !directional_only)
						{
							next_signal_index = i;
						}
						...
						else if(next_signal_working_method == time_interval_with_telegraph || (next_signal_working_method == time_interval && last_longblock_signal_index < INVALID_INDEX))
						{
							...
							{
								if(i > first_stop_signal_index && (last_longblock_signal_index >= INVALID_INDEX) || !next_signal_protects_no_junctions)
								{
									next_signal_index = i;
									count --;
								}
			...
			bool attempt_reservation = directional_only || time_interval_reservation || (next_signal_working_method != time_interval && next_signal_working_method != time_interval_with_telegraph);
				...
				&& !next_signal_protects_no_junctions && this_stop_signal_index == i && !directional_only))
			{
				...
				if(end_of_block && !onward_reservation && !directional_only)
				{
					next_signal_index = last_stop_signal_index;
					break;
				}
	...
	if(!success || (next_signal_index == INVALID_INDEX && onward_reservation == absolute) || !directional_reservation_succeeded || ((next_signal_index < INVALID_INDEX) && (next_signal_working_method == absolute_block || next_signal_working_method == track_circuit_block || next_signal_working_method == cab_signalling || ((next_signal_working_method == time_interval || next_signal_working_method == time_interval_with_telegraph) && !next_signal_protects_no_junctions))))
	{
		...
		if(next_signal_index < INVALID_INDEX && (next_signal_index == start_index || platform_starter) && !onward_reservation)
		{
	...
	{
		if(platform_starter && !onward_reservation && !is_from_starter)
		{
			...
			{
				sint32 success_2 = block_reserver(cnv->get_route(), modified_sighting_distance_tiles, next_signal_index, next_signal_index, 0, true, false, false, none, true); 
