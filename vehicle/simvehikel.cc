		...
		if (!fracht.empty())
		{
			...
			while(iter.next()) 
			{
				...
				else if(halt == end_halt || halt == via_halt) 
				{
					...
					// we want km per hour, so multiply by km_per_tile...
					// EXCEPT that for reasons of stupidity, all displayed speeds are multiplied by 4....
					// ...even in standard... and the actual code is:
					float magic_factor = km_per_tile * 4096 * VEHICLE_SPEED_FACTOR;
